...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link " href="pump-feedback.php">
          <i class="bi bi-question-circle"></i>
          <span>Feedback</span>
        </a>
      </li>
  ...
  <table class="table table-bordered table-lite">
  <thead>
    <tr>
...
$sname="localhost";
 ...
 $q="select * from feedback where license=$license";
 $result=$con->query($q);
 $x=0;
 while ($row = $result->fetch_assoc()) {
    $x=$x+1;
    echo "<tr><th scope='row'>$x</th><td>{$row['date']}</td><td>{$row['email']}</td><td>{$row['rating']}</td><td>{$row['feedback']}</td></tr> ";
}}
?>
  </tbody>
</table>
