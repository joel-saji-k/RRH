  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <li><a class="nav-link " href="mechanic-index.php">
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" href="mechanic-profile.php">
          <i class="bi bi-person"></i>
          <span>Profile</span>
        </a>
...
<?php 
 ...
 $con=mysqli_connect("$sname","$uid","$pwd","$db");
 if($con){
   $q="select * from mechanicbook where email='$uemail'";
 $result=$con->query($q);
 if($result){
 ...
 while ($row = $result->fetch_assoc()) {
  ...
  $q1="INSERT INTO history values($licence,'$semail','Mechanic Service','$uemail','$time',NULL,NULL,'$vtype')";
  $result=$con->query($q1);
  $q2="update mechanicbook set status='processing' where time='$time'";
