...
if(isset($_POST['approve']))
{
  ...
  $q="SELECT * FROM fuel_reg_request where license=$license_id";
  $result=$con->query($q);
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    $q2="SELECT email FROM pump_reg WHERE email='$e'";
    $result2=$con->query($q2);
    if($result2->num_rows > 0){
      echo "<script>window.onload = function() { showPopup('Already approved '); };</script>";
    }
    else{
    $q1="INSERT INTO pump_reg values($l,'$b','$n','$e','$pass','$lat','$lon',0,0,'active','$place')";
    $result=$con->query($q1);
    echo "<script>window.onload = function() { showPopup('Approved'); };</script>";
    $q2="DELETE FROM fuel_reg_request WHERE license = $l";
    $result2=$con->query($q2);
...
if(isset($_POST['reject']))
{
  ...
  $q1="INSERT INTO pump_reject values($l,'$b','$n','$e','$lat','$lon','$place')";
    $result=$con->query($q1);
  $q="DELETE FROM fuel_reg_request where license=$license_id";
  $result=$con->query($q);
    if($result){
...
<head>
  ...
  <!-- Favicons -->
  <link href="/RRH/assets/img/img/favicon.png" rel="icon">
  <link href="/RRH/assets/img/img/apple-touch-icon.png" rel="apple-touch-icon">
  ...
  <link href="/RRH/assets/assets2/assets/vendor/quill/quill.bubble.css" rel="stylesheet">
  <link href="/RRH/assets/assets2/assets/vendor/remixicon/remixicon.css" rel="stylesheet">
  <link href="/RRH/assets/assets2/assets/vendor/simple-datatables/style.css" rel="stylesheet">
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <li><a class="nav-link " href="admin-index.php">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Fuel service</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
